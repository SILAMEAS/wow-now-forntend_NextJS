...
function DisplayNew({ productId }: any) {
    ...
    const fetchProduct = async () => {
      try {
        const response = await axios.get(
          `${API_URL}api/v1/product/get/news-id/${productId}`
        );
    ...
    try {
      if (descriptionImage) {
        const response = await axios.patch(
          `${API_URL}api/v1/product/update/news`,
          formData,
          {
            headers: $headers,
          }
      ...
      router.push((window.location.href = "/all-new"));
    }
  };
  ...
  return (
    <Stack>
      <Stack alignItems={"center"} justifyContent={"center"}>
        ...
        {product.data?.file ? (
          <ShowImage itemData={product.data?.file.slice(0, 1)} />
        ) : (
          <>No New Image</>
        )}
      </Stack>
      <Grid container spacing={2}>
        <Grid item xs={12}>
          <InputFieldNew product={product} setState={setState} state={state} />
          <Stack
            width={"100%"}
            alignItems={"start"}
            direction={"row"}
            ...
            <Grid item xs={6}>
              {productImages.length > 0 ? (
                ...
                >
                  ...
                  <Typography>Drag & Drop or Click to Upload News</Typography>
                </Stack>
              )}
              <input
                accept="image/*"
                  ...
                  <div>
                    ...
                    <Typography>
                      {selectedImageDescription
                        ...
                        : "Drag & Drop or Click to Upload News Description"}
                    </Typography>
                  </div>
                </Stack>
              )}
      ...
      <Stack justifyContent={"center"} alignItems={"center"} py={3}>
        <Typography sx={{ ...ConstantStyle.text.titlePage, py: 10 }}>
          Image of News Description
        </Typography>

        {product.data?.file?.length > 1 ? (
          <ShowImageDes itemData={product.data?.file.slice(1)} />
        ) : (
          <>No File News Descriptions Image</>
        )}
      </Stack>
      <Button
        type="submit"
